...
const TipoCombustible = () => {
    ...
    // Editar
    const handleEdit = (id) => {
        const marca = combustibleList.find(v => v.id === id);
        setEditValues({ id: marca.id, descripcion: marca.descripcion });
        setEditOpen(true);
    };

    const handleEditSubmit = () => {
        Axios.put(`http://localhost:3001/combustibles/${editValues.id}`, {
    ...
    const handleDelete = (id) => {
        Axios.delete(`http://localhost:3001/combustibles/${id}`)
            .then((response) => {
                getCombustibles();
            })
            .catch((error) => {
    ...
    const handleFormSubmit = (values, {resetForm}) => {
        Axios.post("http://localhost:3001/combustibles", {
    ...
    //Excel
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(combustibleList);
    ...
    return(
        <Box m="20px">
            <Box display="flex" justifyContent="center" mt="20px">
                {showAlert && (
                    <Alert icon={<CheckIcon fontSize="inherit" />} severity="success">
                        Tipo de combustible agregado correctamente.
                    </Alert>
                )}
            </Box>
            <Header title="Combustibles" subtitle = "Gestion de combustibles"></Header>
            ...
            <Box m="5px"></Box>
            <DataGrid rows={combustibleList} columns={columns} autoHeight/>
            </Box>
        </Box>
    );
};
