...
const TipoVehiculo = () => {
    ...
    // Editar
    const handleEdit = (id) => {
        const vehiculo = typevehiculoList.find(v => v.id === id);
        setEditValues({ id: vehiculo.id, descripcion: vehiculo.descripcion });
        setEditOpen(true);
    };

    const handleEditSubmit = () => {
        Axios.put(`http://localhost:3001/typevehiculos/${editValues.id}`, {
    ...
    const handleDelete = (id) => {
        Axios.delete(`http://localhost:3001/typevehiculos/${id}`)
            .then((response) => {
                getTypevehiculos();
            })
            .catch((error) => {
    ...
    const handleFormSubmit = (values, {resetForm}) => {
        Axios.post("http://localhost:3001/typevehiculos", {
    ...
    //Excel
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(typevehiculoList);
    ...
    return(
        <Box m="20px">
            <Box display="flex" justifyContent="center" mt="20px">
                {showAlert && (
                    <Alert icon={<CheckIcon fontSize="inherit" />} severity="success">
                        Tipo de vehículo agregado correctamente.
                    </Alert>
                )}
            </Box>
            <Header title="Tipo de vehiculo" subtitle = "Gestion tipo de vehiculo"></Header>
            ...
            <Box m="5px"></Box>
            <DataGrid rows={typevehiculoList} columns={columns} autoHeight/>
            </Box>
        </Box>
    );
};
